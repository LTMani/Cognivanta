/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: INTERCOMCONVERSATIONTOPIC
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface IntercomConversationTopicAttributes {
  id: string;
  name: string;
...
export class IntercomConversationTopic {
  private attributes: IntercomConversationTopicAttributes;

  constructor(attrs?: Partial<IntercomConversationTopicAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'IntercomConversationTopic Standard Configuration',
  ...
  public setState(state: IntercomConversationTopicAttributes['state']): this {
  ...
  public toJSON(): IntercomConversationTopicAttributes {

/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: DATAQUALITYMETRIC
 * ============================================================================
 * Description: Data quality metric with completeness and freshness scores
...
export interface DataQualityMetricAttributes {
  id: string;
  name: string;
...
export class DataQualityMetric {
  private attributes: DataQualityMetricAttributes;

  constructor(attributes?: Partial<DataQualityMetricAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attributes?.id || generateUUID(),
      name: attributes?.name || 'DataQualityMetric Instance',
  ...
  public update(attributes: Partial<DataQualityMetricAttributes>): this {
    this.attributes = {
      ...this.attributes,
      ...attributes,
  ...
  public setStatus(status: DataQualityMetricAttributes['status']): this {
  ...
  public toJSON(): DataQualityMetricAttributes {
    return { ...this.attributes };
  }

  public static fromJSON(json: Partial<DataQualityMetricAttributes>): DataQualityMetric {
    return new DataQualityMetric(json);
  }

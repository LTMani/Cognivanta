/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: DATASETARTIFACTRECORD
 * ============================================================================
 * Description: Training / evaluation dataset artifact with checksum
...
export interface DatasetArtifactRecordAttributes {
  id: string;
  name: string;
...
export class DatasetArtifactRecord {
  private attributes: DatasetArtifactRecordAttributes;

  constructor(attributes?: Partial<DatasetArtifactRecordAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attributes?.id || generateUUID(),
      name: attributes?.name || 'DatasetArtifactRecord Instance',
  ...
  public update(attributes: Partial<DatasetArtifactRecordAttributes>): this {
    this.attributes = {
      ...this.attributes,
      ...attributes,
  ...
  public setStatus(status: DatasetArtifactRecordAttributes['status']): this {
  ...
  public toJSON(): DatasetArtifactRecordAttributes {
    return { ...this.attributes };
  }

  public static fromJSON(json: Partial<DatasetArtifactRecordAttributes>): DatasetArtifactRecord {
    return new DatasetArtifactRecord(json);
  }

/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: PRIORITYJOBQUEUERECORD
 * ============================================================================
 * Description: Priority async job queue record with retry backoff state
...
export interface PriorityJobQueueRecordAttributes {
  id: string;
  name: string;
...
export class PriorityJobQueueRecord {
  private attributes: PriorityJobQueueRecordAttributes;

  constructor(attributes?: Partial<PriorityJobQueueRecordAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attributes?.id || generateUUID(),
      name: attributes?.name || 'PriorityJobQueueRecord Instance',
  ...
  public update(attributes: Partial<PriorityJobQueueRecordAttributes>): this {
    this.attributes = {
      ...this.attributes,
      ...attributes,
  ...
  public setStatus(status: PriorityJobQueueRecordAttributes['status']): this {
  ...
  public toJSON(): PriorityJobQueueRecordAttributes {
    return { ...this.attributes };
  }

  public static fromJSON(json: Partial<PriorityJobQueueRecordAttributes>): PriorityJobQueueRecord {
    return new PriorityJobQueueRecord(json);
  }

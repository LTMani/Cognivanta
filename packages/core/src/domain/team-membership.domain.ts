/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: TEAMMEMBERSHIP
 * ============================================================================
 * Description: Organization team membership with granular permissions
...
export class TeamMembership {
  private attributes: TeamMembershipAttributes;

  constructor(attributes?: Partial<TeamMembershipAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attributes?.id || generateUUID(),
      name: attributes?.name || 'TeamMembership Instance',
  ...
  public update(attributes: Partial<TeamMembershipAttributes>): this {
    this.attributes = {
      ...this.attributes,
      ...attributes,
  ...
  public setStatus(status: TeamMembershipAttributes['status']): this {
  ...
  public toJSON(): TeamMembershipAttributes {
    return { ...this.attributes };
  }

  public static fromJSON(json: Partial<TeamMembershipAttributes>): TeamMembership {
    return new TeamMembership(json);
  }

/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: AGENTEXECUTIONRUN
 * ============================================================================
 * Description: Execution run of an autonomous agent with step logs
...
export interface AgentExecutionRunAttributes {
  id: string;
  name: string;
...
export class AgentExecutionRun {
  private attributes: AgentExecutionRunAttributes;

  constructor(attributes?: Partial<AgentExecutionRunAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attributes?.id || generateUUID(),
      name: attributes?.name || 'AgentExecutionRun Instance',
  ...
  public update(attributes: Partial<AgentExecutionRunAttributes>): this {
    this.attributes = {
      ...this.attributes,
      ...attributes,
  ...
  public setStatus(status: AgentExecutionRunAttributes['status']): this {
  ...
  public toJSON(): AgentExecutionRunAttributes {
    return { ...this.attributes };
  }

  public static fromJSON(json: Partial<AgentExecutionRunAttributes>): AgentExecutionRun {
    return new AgentExecutionRun(json);
  }

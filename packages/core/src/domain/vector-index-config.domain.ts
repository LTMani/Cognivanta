/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: VECTORINDEXCONFIG
 * ============================================================================
 * Description: HNSW / pgvector configuration with distance metrics and dimensions
...
export interface VectorIndexConfigAttributes {
  id: string;
  name: string;
...
export class VectorIndexConfig {
  private attributes: VectorIndexConfigAttributes;

  constructor(attributes?: Partial<VectorIndexConfigAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attributes?.id || generateUUID(),
      name: attributes?.name || 'VectorIndexConfig Instance',
  ...
  public update(attributes: Partial<VectorIndexConfigAttributes>): this {
    this.attributes = {
      ...this.attributes,
      ...attributes,
  ...
  public setStatus(status: VectorIndexConfigAttributes['status']): this {
  ...
  public toJSON(): VectorIndexConfigAttributes {
    return { ...this.attributes };
  }

  public static fromJSON(json: Partial<VectorIndexConfigAttributes>): VectorIndexConfig {
    return new VectorIndexConfig(json);
  }

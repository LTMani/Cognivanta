/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: EMAILSMTPSERVERCONFIG
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface EmailSMTPServerConfigAttributes {
  id: string;
  name: string;
...
export class EmailSMTPServerConfig {
  private attributes: EmailSMTPServerConfigAttributes;

  constructor(attrs?: Partial<EmailSMTPServerConfigAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'EmailSMTPServerConfig Default Item',
  ...
  public setStatus(status: EmailSMTPServerConfigAttributes['status']): this {
  ...
  public toJSON(): EmailSMTPServerConfigAttributes {

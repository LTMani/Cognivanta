/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: STRIPESUBSCRIPTIONCUSTOMER
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface StripeSubscriptionCustomerAttributes {
  id: string;
  name: string;
...
export class StripeSubscriptionCustomer {
  private attributes: StripeSubscriptionCustomerAttributes;

  constructor(attrs?: Partial<StripeSubscriptionCustomerAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'StripeSubscriptionCustomer Standard Configuration',
  ...
  public setState(state: StripeSubscriptionCustomerAttributes['state']): this {
  ...
  public toJSON(): StripeSubscriptionCustomerAttributes {

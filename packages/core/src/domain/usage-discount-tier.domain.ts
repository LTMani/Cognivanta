/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: USAGEDISCOUNTTIER
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface UsageDiscountTierAttributes {
  id: string;
  name: string;
...
export class UsageDiscountTier {
  private attributes: UsageDiscountTierAttributes;

  constructor(attrs?: Partial<UsageDiscountTierAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'UsageDiscountTier Default Item',
  ...
  public setStatus(status: UsageDiscountTierAttributes['status']): this {
  ...
  public toJSON(): UsageDiscountTierAttributes {

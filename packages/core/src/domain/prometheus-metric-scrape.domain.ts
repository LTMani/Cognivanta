/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: PROMETHEUSMETRICSCRAPE
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface PrometheusMetricScrapeAttributes {
  id: string;
  name: string;
...
export class PrometheusMetricScrape {
  private attributes: PrometheusMetricScrapeAttributes;

  constructor(attrs?: Partial<PrometheusMetricScrapeAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'PrometheusMetricScrape Default Item',
  ...
  public setStatus(status: PrometheusMetricScrapeAttributes['status']): this {
  ...
  public toJSON(): PrometheusMetricScrapeAttributes {

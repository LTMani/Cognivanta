/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: DOCUMENTCHUNK
 * ============================================================================
 * Description: Text chunk with vector embedding, BM25 tokens, and coordinates
...
export class DocumentChunk {
  private attributes: DocumentChunkAttributes;

  constructor(attributes?: Partial<DocumentChunkAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attributes?.id || generateUUID(),
      name: attributes?.name || 'DocumentChunk Instance',
  ...
  public update(attributes: Partial<DocumentChunkAttributes>): this {
    this.attributes = {
      ...this.attributes,
      ...attributes,
  ...
  public setStatus(status: DocumentChunkAttributes['status']): this {
  ...
  public toJSON(): DocumentChunkAttributes {
    return { ...this.attributes };
  }

  public static fromJSON(json: Partial<DocumentChunkAttributes>): DocumentChunk {
    return new DocumentChunk(json);
  }

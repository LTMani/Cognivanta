/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: PROMPTTEMPLATEVERSION
 * ============================================================================
 * Description: Versioned prompt template with variable schemas and test runs
...
export interface PromptTemplateVersionAttributes {
  id: string;
  name: string;
...
export class PromptTemplateVersion {
  private attributes: PromptTemplateVersionAttributes;

  constructor(attributes?: Partial<PromptTemplateVersionAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attributes?.id || generateUUID(),
      name: attributes?.name || 'PromptTemplateVersion Instance',
  ...
  public update(attributes: Partial<PromptTemplateVersionAttributes>): this {
    this.attributes = {
      ...this.attributes,
      ...attributes,
  ...
  public setStatus(status: PromptTemplateVersionAttributes['status']): this {
  ...
  public toJSON(): PromptTemplateVersionAttributes {
    return { ...this.attributes };
  }

  public static fromJSON(json: Partial<PromptTemplateVersionAttributes>): PromptTemplateVersion {
    return new PromptTemplateVersion(json);
  }

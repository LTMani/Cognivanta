/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: TERRAFORMSTATESNAPSHOT
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface TerraformStateSnapshotAttributes {
  id: string;
  name: string;
...
export class TerraformStateSnapshot {
  private attributes: TerraformStateSnapshotAttributes;

  constructor(attrs?: Partial<TerraformStateSnapshotAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'TerraformStateSnapshot Standard Configuration',
  ...
  public setState(state: TerraformStateSnapshotAttributes['state']): this {
  ...
  public toJSON(): TerraformStateSnapshotAttributes {

/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: EVALUATIONMETRICRESULT
 * ============================================================================
 * Description: Evaluation run result with ROUGE, BLEU, and faithfulness scores
...
export interface EvaluationMetricResultAttributes {
  id: string;
  name: string;
...
export class EvaluationMetricResult {
  private attributes: EvaluationMetricResultAttributes;

  constructor(attributes?: Partial<EvaluationMetricResultAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attributes?.id || generateUUID(),
      name: attributes?.name || 'EvaluationMetricResult Instance',
  ...
  public update(attributes: Partial<EvaluationMetricResultAttributes>): this {
    this.attributes = {
      ...this.attributes,
      ...attributes,
  ...
  public setStatus(status: EvaluationMetricResultAttributes['status']): this {
  ...
  public toJSON(): EvaluationMetricResultAttributes {
    return { ...this.attributes };
  }

  public static fromJSON(json: Partial<EvaluationMetricResultAttributes>): EvaluationMetricResult {
    return new EvaluationMetricResult(json);
  }

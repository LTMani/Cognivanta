/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: DOCUMENTRECORD
 * ============================================================================
 * Description: Ingested enterprise document with parser metadata and hashes
...
export class DocumentRecord {
  private attributes: DocumentRecordAttributes;

  constructor(attributes?: Partial<DocumentRecordAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attributes?.id || generateUUID(),
      name: attributes?.name || 'DocumentRecord Instance',
  ...
  public update(attributes: Partial<DocumentRecordAttributes>): this {
    this.attributes = {
      ...this.attributes,
      ...attributes,
  ...
  public setStatus(status: DocumentRecordAttributes['status']): this {
  ...
  public toJSON(): DocumentRecordAttributes {
    return { ...this.attributes };
  }

  public static fromJSON(json: Partial<DocumentRecordAttributes>): DocumentRecord {
    return new DocumentRecord(json);
  }

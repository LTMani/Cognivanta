/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: PAGERDUTYALERTTARGET
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface PagerDutyAlertTargetAttributes {
  id: string;
  name: string;
...
export class PagerDutyAlertTarget {
  private attributes: PagerDutyAlertTargetAttributes;

  constructor(attrs?: Partial<PagerDutyAlertTargetAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'PagerDutyAlertTarget Default Item',
  ...
  public setStatus(status: PagerDutyAlertTargetAttributes['status']): this {
  ...
  public toJSON(): PagerDutyAlertTargetAttributes {

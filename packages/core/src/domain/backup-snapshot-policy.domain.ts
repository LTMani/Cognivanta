/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: BACKUPSNAPSHOTPOLICY
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface BackupSnapshotPolicyAttributes {
  id: string;
  name: string;
...
export class BackupSnapshotPolicy {
  private attributes: BackupSnapshotPolicyAttributes;

  constructor(attrs?: Partial<BackupSnapshotPolicyAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'BackupSnapshotPolicy Default Item',
  ...
  public setStatus(status: BackupSnapshotPolicyAttributes['status']): this {
  ...
  public toJSON(): BackupSnapshotPolicyAttributes {

/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: PUBSUBSUBSCRIPTIONITEM
 * ============================================================================
 * Description: PubSub channel subscription item with consumer offsets
...
export interface PubSubSubscriptionItemAttributes {
  id: string;
  name: string;
...
export class PubSubSubscriptionItem {
  private attributes: PubSubSubscriptionItemAttributes;

  constructor(attributes?: Partial<PubSubSubscriptionItemAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attributes?.id || generateUUID(),
      name: attributes?.name || 'PubSubSubscriptionItem Instance',
  ...
  public update(attributes: Partial<PubSubSubscriptionItemAttributes>): this {
    this.attributes = {
      ...this.attributes,
      ...attributes,
  ...
  public setStatus(status: PubSubSubscriptionItemAttributes['status']): this {
  ...
  public toJSON(): PubSubSubscriptionItemAttributes {
    return { ...this.attributes };
  }

  public static fromJSON(json: Partial<PubSubSubscriptionItemAttributes>): PubSubSubscriptionItem {
    return new PubSubSubscriptionItem(json);
  }

/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: POSTHOGFEATUREFLAGVARIANT
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface PostHogFeatureFlagVariantAttributes {
  id: string;
  name: string;
...
export class PostHogFeatureFlagVariant {
  private attributes: PostHogFeatureFlagVariantAttributes;

  constructor(attrs?: Partial<PostHogFeatureFlagVariantAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'PostHogFeatureFlagVariant Standard Configuration',
  ...
  public setState(state: PostHogFeatureFlagVariantAttributes['state']): this {
  ...
  public toJSON(): PostHogFeatureFlagVariantAttributes {

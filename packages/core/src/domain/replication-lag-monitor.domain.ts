/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: REPLICATIONLAGMONITOR
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface ReplicationLagMonitorAttributes {
  id: string;
  name: string;
...
export class ReplicationLagMonitor {
  private attributes: ReplicationLagMonitorAttributes;

  constructor(attrs?: Partial<ReplicationLagMonitorAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'ReplicationLagMonitor Default Item',
  ...
  public setStatus(status: ReplicationLagMonitorAttributes['status']): this {
  ...
  public toJSON(): ReplicationLagMonitorAttributes {

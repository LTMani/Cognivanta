/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: GUARDRAILSECURITYPOLICY
 * ============================================================================
 * Description: Security guardrail policy with DLP rules and toxicity thresholds
...
export interface GuardrailSecurityPolicyAttributes {
  id: string;
  name: string;
...
export class GuardrailSecurityPolicy {
  private attributes: GuardrailSecurityPolicyAttributes;

  constructor(attributes?: Partial<GuardrailSecurityPolicyAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attributes?.id || generateUUID(),
      name: attributes?.name || 'GuardrailSecurityPolicy Instance',
  ...
  public update(attributes: Partial<GuardrailSecurityPolicyAttributes>): this {
    this.attributes = {
      ...this.attributes,
      ...attributes,
  ...
  public setStatus(status: GuardrailSecurityPolicyAttributes['status']): this {
  ...
  public toJSON(): GuardrailSecurityPolicyAttributes {
    return { ...this.attributes };
  }

  public static fromJSON(json: Partial<GuardrailSecurityPolicyAttributes>): GuardrailSecurityPolicy {
    return new GuardrailSecurityPolicy(json);
  }

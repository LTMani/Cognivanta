/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: BUGSNAGERRORDIAGNOSTICLOG
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface BugsnagErrorDiagnosticLogAttributes {
  id: string;
  name: string;
...
export class BugsnagErrorDiagnosticLog {
  private attributes: BugsnagErrorDiagnosticLogAttributes;

  constructor(attrs?: Partial<BugsnagErrorDiagnosticLogAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'BugsnagErrorDiagnosticLog Standard Configuration',
  ...
  public setState(state: BugsnagErrorDiagnosticLogAttributes['state']): this {
  ...
  public toJSON(): BugsnagErrorDiagnosticLogAttributes {

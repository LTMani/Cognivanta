/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: AWSCLOUDWATCHALARM
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface AWSCloudWatchAlarmAttributes {
  id: string;
  name: string;
...
export class AWSCloudWatchAlarm {
  private attributes: AWSCloudWatchAlarmAttributes;

  constructor(attrs?: Partial<AWSCloudWatchAlarmAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'AWSCloudWatchAlarm Standard Configuration',
  ...
  public setState(state: AWSCloudWatchAlarmAttributes['state']): this {
  ...
  public toJSON(): AWSCloudWatchAlarmAttributes {

/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: AWSKMSCUSTOMERMASTERKEY
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface AWSKMSCustomerMasterKeyAttributes {
  id: string;
  name: string;
...
export class AWSKMSCustomerMasterKey {
  private attributes: AWSKMSCustomerMasterKeyAttributes;

  constructor(attrs?: Partial<AWSKMSCustomerMasterKeyAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'AWSKMSCustomerMasterKey Standard Configuration',
  ...
  public setState(state: AWSKMSCustomerMasterKeyAttributes['state']): this {
  ...
  public toJSON(): AWSKMSCustomerMasterKeyAttributes {

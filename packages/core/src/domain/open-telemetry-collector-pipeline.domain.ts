/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: OPENTELEMETRYCOLLECTORPIPELINE
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface OpenTelemetryCollectorPipelineAttributes {
  id: string;
  name: string;
...
export class OpenTelemetryCollectorPipeline {
  private attributes: OpenTelemetryCollectorPipelineAttributes;

  constructor(attrs?: Partial<OpenTelemetryCollectorPipelineAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'OpenTelemetryCollectorPipeline Standard Configuration',
  ...
  public setState(state: OpenTelemetryCollectorPipelineAttributes['state']): this {
  ...
  public toJSON(): OpenTelemetryCollectorPipelineAttributes {

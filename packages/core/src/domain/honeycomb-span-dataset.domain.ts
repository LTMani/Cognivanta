/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: HONEYCOMBSPANDATASET
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface HoneycombSpanDatasetAttributes {
  id: string;
  name: string;
...
export class HoneycombSpanDataset {
  private attributes: HoneycombSpanDatasetAttributes;

  constructor(attrs?: Partial<HoneycombSpanDatasetAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'HoneycombSpanDataset Standard Configuration',
  ...
  public setState(state: HoneycombSpanDatasetAttributes['state']): this {
  ...
  public toJSON(): HoneycombSpanDatasetAttributes {

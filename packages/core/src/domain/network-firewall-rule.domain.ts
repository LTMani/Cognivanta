/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: NETWORKFIREWALLRULE
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface NetworkFirewallRuleAttributes {
  id: string;
  name: string;
...
export class NetworkFirewallRule {
  private attributes: NetworkFirewallRuleAttributes;

  constructor(attrs?: Partial<NetworkFirewallRuleAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'NetworkFirewallRule Default Item',
  ...
  public setStatus(status: NetworkFirewallRuleAttributes['status']): this {
  ...
  public toJSON(): NetworkFirewallRuleAttributes {

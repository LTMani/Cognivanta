/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: LDAPDIRECTORYSYNC
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface LDAPDirectorySyncAttributes {
  id: string;
  name: string;
...
export class LDAPDirectorySync {
  private attributes: LDAPDirectorySyncAttributes;

  constructor(attrs?: Partial<LDAPDirectorySyncAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'LDAPDirectorySync Default Item',
  ...
  public setStatus(status: LDAPDirectorySyncAttributes['status']): this {
  ...
  public toJSON(): LDAPDirectorySyncAttributes {

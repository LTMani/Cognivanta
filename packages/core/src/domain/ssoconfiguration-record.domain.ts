/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: SSOCONFIGURATIONRECORD
 * ============================================================================
 * Description: SAML 2.0 / OIDC enterprise SSO configuration record
...
export interface SSOConfigurationRecordAttributes {
  id: string;
  name: string;
...
export class SSOConfigurationRecord {
  private attributes: SSOConfigurationRecordAttributes;

  constructor(attributes?: Partial<SSOConfigurationRecordAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attributes?.id || generateUUID(),
      name: attributes?.name || 'SSOConfigurationRecord Instance',
  ...
  public update(attributes: Partial<SSOConfigurationRecordAttributes>): this {
    this.attributes = {
      ...this.attributes,
      ...attributes,
  ...
  public setStatus(status: SSOConfigurationRecordAttributes['status']): this {
  ...
  public toJSON(): SSOConfigurationRecordAttributes {
    return { ...this.attributes };
  }

  public static fromJSON(json: Partial<SSOConfigurationRecordAttributes>): SSOConfigurationRecord {
    return new SSOConfigurationRecord(json);
  }

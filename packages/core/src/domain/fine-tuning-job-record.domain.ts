/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: FINETUNINGJOBRECORD
 * ============================================================================
 * Description: Fine-tuning job record with LoRA hyperparameters and loss curves
...
export interface FineTuningJobRecordAttributes {
  id: string;
  name: string;
...
export class FineTuningJobRecord {
  private attributes: FineTuningJobRecordAttributes;

  constructor(attributes?: Partial<FineTuningJobRecordAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attributes?.id || generateUUID(),
      name: attributes?.name || 'FineTuningJobRecord Instance',
  ...
  public update(attributes: Partial<FineTuningJobRecordAttributes>): this {
    this.attributes = {
      ...this.attributes,
      ...attributes,
  ...
  public setStatus(status: FineTuningJobRecordAttributes['status']): this {
  ...
  public toJSON(): FineTuningJobRecordAttributes {
    return { ...this.attributes };
  }

  public static fromJSON(json: Partial<FineTuningJobRecordAttributes>): FineTuningJobRecord {
    return new FineTuningJobRecord(json);
  }

/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: VAULTTRANSITSECRETKEY
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface VaultTransitSecretKeyAttributes {
  id: string;
  name: string;
...
export class VaultTransitSecretKey {
  private attributes: VaultTransitSecretKeyAttributes;

  constructor(attrs?: Partial<VaultTransitSecretKeyAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'VaultTransitSecretKey Standard Configuration',
  ...
  public setState(state: VaultTransitSecretKeyAttributes['state']): this {
  ...
  public toJSON(): VaultTransitSecretKeyAttributes {

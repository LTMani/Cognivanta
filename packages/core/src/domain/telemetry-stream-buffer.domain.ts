/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: TELEMETRYSTREAMBUFFER
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface TelemetryStreamBufferAttributes {
  id: string;
  name: string;
...
export class TelemetryStreamBuffer {
  private attributes: TelemetryStreamBufferAttributes;

  constructor(attrs?: Partial<TelemetryStreamBufferAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'TelemetryStreamBuffer Default Item',
  ...
  public setStatus(status: TelemetryStreamBufferAttributes['status']): this {
  ...
  public toJSON(): TelemetryStreamBufferAttributes {

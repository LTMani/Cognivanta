/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: PADDLECHECKOUTTRANSACTION
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface PaddleCheckoutTransactionAttributes {
  id: string;
  name: string;
...
export class PaddleCheckoutTransaction {
  private attributes: PaddleCheckoutTransactionAttributes;

  constructor(attrs?: Partial<PaddleCheckoutTransactionAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'PaddleCheckoutTransaction Standard Configuration',
  ...
  public setState(state: PaddleCheckoutTransactionAttributes['state']): this {
  ...
  public toJSON(): PaddleCheckoutTransactionAttributes {

/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: WORKFLOWEXECUTIONTRACE
 * ============================================================================
 * Description: Execution trace of a workflow run with node telemetry
...
export interface WorkflowExecutionTraceAttributes {
  id: string;
  name: string;
...
export class WorkflowExecutionTrace {
  private attributes: WorkflowExecutionTraceAttributes;

  constructor(attributes?: Partial<WorkflowExecutionTraceAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attributes?.id || generateUUID(),
      name: attributes?.name || 'WorkflowExecutionTrace Instance',
  ...
  public update(attributes: Partial<WorkflowExecutionTraceAttributes>): this {
    this.attributes = {
      ...this.attributes,
      ...attributes,
  ...
  public setStatus(status: WorkflowExecutionTraceAttributes['status']): this {
  ...
  public toJSON(): WorkflowExecutionTraceAttributes {
    return { ...this.attributes };
  }

  public static fromJSON(json: Partial<WorkflowExecutionTraceAttributes>): WorkflowExecutionTrace {
    return new WorkflowExecutionTrace(json);
  }

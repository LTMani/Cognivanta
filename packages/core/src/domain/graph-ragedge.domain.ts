/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: GRAPHRAGEDGE
 * ============================================================================
 * Description: Knowledge graph directed relationship edge with weights
...
export interface GraphRAGEdgeAttributes {
  id: string;
  name: string;
...
export class GraphRAGEdge {
  private attributes: GraphRAGEdgeAttributes;

  constructor(attributes?: Partial<GraphRAGEdgeAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attributes?.id || generateUUID(),
      name: attributes?.name || 'GraphRAGEdge Instance',
  ...
  public update(attributes: Partial<GraphRAGEdgeAttributes>): this {
    this.attributes = {
      ...this.attributes,
      ...attributes,
  ...
  public setStatus(status: GraphRAGEdgeAttributes['status']): this {
  ...
  public toJSON(): GraphRAGEdgeAttributes {
    return { ...this.attributes };
  }

  public static fromJSON(json: Partial<GraphRAGEdgeAttributes>): GraphRAGEdge {
    return new GraphRAGEdge(json);
  }

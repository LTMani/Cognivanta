/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: STORAGEVOLUMEMOUNT
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface StorageVolumeMountAttributes {
  id: string;
  name: string;
...
export class StorageVolumeMount {
  private attributes: StorageVolumeMountAttributes;

  constructor(attrs?: Partial<StorageVolumeMountAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'StorageVolumeMount Default Item',
  ...
  public setStatus(status: StorageVolumeMountAttributes['status']): this {
  ...
  public toJSON(): StorageVolumeMountAttributes {

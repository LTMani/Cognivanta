/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: TRACINGSPANRECORD
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface TracingSpanRecordAttributes {
  id: string;
  name: string;
...
export class TracingSpanRecord {
  private attributes: TracingSpanRecordAttributes;

  constructor(attrs?: Partial<TracingSpanRecordAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'TracingSpanRecord Default Item',
  ...
  public setStatus(status: TracingSpanRecordAttributes['status']): this {
  ...
  public toJSON(): TracingSpanRecordAttributes {

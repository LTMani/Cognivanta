/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: DOCKERCONTAINERSPEC
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface DockerContainerSpecAttributes {
  id: string;
  name: string;
...
export class DockerContainerSpec {
  private attributes: DockerContainerSpecAttributes;

  constructor(attrs?: Partial<DockerContainerSpecAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'DockerContainerSpec Standard Configuration',
  ...
  public setState(state: DockerContainerSpecAttributes['state']): this {
  ...
  public toJSON(): DockerContainerSpecAttributes {

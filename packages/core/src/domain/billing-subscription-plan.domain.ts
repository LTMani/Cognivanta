...
export class BillingSubscriptionPlan {
  private attributes: BillingSubscriptionPlanAttributes;

  constructor(attrs?: Partial<BillingSubscriptionPlanAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'BillingSubscriptionPlan Default Item',
  ...
  public setStatus(status: BillingSubscriptionPlanAttributes['status']): this {

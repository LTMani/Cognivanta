/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: PINGIDENTITYOIDCCONFIGURATION
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface PingIdentityOIDCConfigurationAttributes {
  id: string;
  name: string;
...
export class PingIdentityOIDCConfiguration {
  private attributes: PingIdentityOIDCConfigurationAttributes;

  constructor(attrs?: Partial<PingIdentityOIDCConfigurationAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'PingIdentityOIDCConfiguration Standard Configuration',
  ...
  public setState(state: PingIdentityOIDCConfigurationAttributes['state']): this {
  ...
  public toJSON(): PingIdentityOIDCConfigurationAttributes {

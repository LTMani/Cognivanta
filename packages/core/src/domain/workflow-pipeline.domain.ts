 ...
 * ============================================================================
 * Description: Visual DAG workflow pipeline definition and node graph
...
export class WorkflowPipeline {
  private attributes: WorkflowPipelineAttributes;

  constructor(attributes?: Partial<WorkflowPipelineAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attributes?.id || generateUUID(),
      name: attributes?.name || 'WorkflowPipeline Instance',
  ...
  public update(attributes: Partial<WorkflowPipelineAttributes>): this {
    this.attributes = {
      ...this.attributes,
      ...attributes,
  ...
  public setStatus(status: WorkflowPipelineAttributes['status']): this {
  ...
  public static fromJSON(json: Partial<WorkflowPipelineAttributes>): WorkflowPipeline {
    return new WorkflowPipeline(json);
  }

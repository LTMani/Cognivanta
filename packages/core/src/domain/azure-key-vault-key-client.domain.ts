/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: AZUREKEYVAULTKEYCLIENT
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface AzureKeyVaultKeyClientAttributes {
  id: string;
  name: string;
...
export class AzureKeyVaultKeyClient {
  private attributes: AzureKeyVaultKeyClientAttributes;

  constructor(attrs?: Partial<AzureKeyVaultKeyClientAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'AzureKeyVaultKeyClient Standard Configuration',
  ...
  public setState(state: AzureKeyVaultKeyClientAttributes['state']): this {
  ...
  public toJSON(): AzureKeyVaultKeyClientAttributes {

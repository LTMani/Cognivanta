/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: SENDGRIDEMAILTEMPLATERECORD
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface SendGridEmailTemplateRecordAttributes {
  id: string;
  name: string;
...
export class SendGridEmailTemplateRecord {
  private attributes: SendGridEmailTemplateRecordAttributes;

  constructor(attrs?: Partial<SendGridEmailTemplateRecordAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'SendGridEmailTemplateRecord Standard Configuration',
  ...
  public setState(state: SendGridEmailTemplateRecordAttributes['state']): this {
  ...
  public toJSON(): SendGridEmailTemplateRecordAttributes {

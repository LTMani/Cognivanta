/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: TENANTFEATUREFLAG
 * ============================================================================
 * Description: Tenant-specific feature flag with percentage rollout
...
export interface TenantFeatureFlagAttributes {
  id: string;
  name: string;
...
export class TenantFeatureFlag {
  private attributes: TenantFeatureFlagAttributes;

  constructor(attributes?: Partial<TenantFeatureFlagAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attributes?.id || generateUUID(),
      name: attributes?.name || 'TenantFeatureFlag Instance',
  ...
  public update(attributes: Partial<TenantFeatureFlagAttributes>): this {
    this.attributes = {
      ...this.attributes,
      ...attributes,
  ...
  public setStatus(status: TenantFeatureFlagAttributes['status']): this {
  ...
  public toJSON(): TenantFeatureFlagAttributes {
    return { ...this.attributes };
  }

  public static fromJSON(json: Partial<TenantFeatureFlagAttributes>): TenantFeatureFlag {
    return new TenantFeatureFlag(json);
  }

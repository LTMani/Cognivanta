/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: WORKSPACEENVIRONMENT
 * ============================================================================
 * Description: Isolated workspace environment with connector bindings
...
export interface WorkspaceEnvironmentAttributes {
  id: string;
  name: string;
...
export class WorkspaceEnvironment {
  private attributes: WorkspaceEnvironmentAttributes;

  constructor(attributes?: Partial<WorkspaceEnvironmentAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attributes?.id || generateUUID(),
      name: attributes?.name || 'WorkspaceEnvironment Instance',
  ...
  public update(attributes: Partial<WorkspaceEnvironmentAttributes>): this {
    this.attributes = {
      ...this.attributes,
      ...attributes,
  ...
  public setStatus(status: WorkspaceEnvironmentAttributes['status']): this {
  ...
  public toJSON(): WorkspaceEnvironmentAttributes {
    return { ...this.attributes };
  }

  public static fromJSON(json: Partial<WorkspaceEnvironmentAttributes>): WorkspaceEnvironment {
    return new WorkspaceEnvironment(json);
  }

/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: HEAPSESSIONANALYTICSTRACKER
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface HeapSessionAnalyticsTrackerAttributes {
  id: string;
  name: string;
...
export class HeapSessionAnalyticsTracker {
  private attributes: HeapSessionAnalyticsTrackerAttributes;

  constructor(attrs?: Partial<HeapSessionAnalyticsTrackerAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'HeapSessionAnalyticsTracker Standard Configuration',
  ...
  public setState(state: HeapSessionAnalyticsTrackerAttributes['state']): this {
  ...
  public toJSON(): HeapSessionAnalyticsTrackerAttributes {

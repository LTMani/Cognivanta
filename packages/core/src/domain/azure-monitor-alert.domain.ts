/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: AZUREMONITORALERT
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface AzureMonitorAlertAttributes {
  id: string;
  name: string;
...
export class AzureMonitorAlert {
  private attributes: AzureMonitorAlertAttributes;

  constructor(attrs?: Partial<AzureMonitorAlertAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'AzureMonitorAlert Standard Configuration',
  ...
  public setState(state: AzureMonitorAlertAttributes['state']): this {
  ...
  public toJSON(): AzureMonitorAlertAttributes {

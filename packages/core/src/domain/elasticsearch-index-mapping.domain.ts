/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: ELASTICSEARCHINDEXMAPPING
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface ElasticsearchIndexMappingAttributes {
  id: string;
  name: string;
...
export class ElasticsearchIndexMapping {
  private attributes: ElasticsearchIndexMappingAttributes;

  constructor(attrs?: Partial<ElasticsearchIndexMappingAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'ElasticsearchIndexMapping Standard Configuration',
  ...
  public setState(state: ElasticsearchIndexMappingAttributes['state']): this {
  ...
  public toJSON(): ElasticsearchIndexMappingAttributes {

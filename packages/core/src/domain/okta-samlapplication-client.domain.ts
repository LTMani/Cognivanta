/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: OKTASAMLAPPLICATIONCLIENT
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface OktaSAMLApplicationClientAttributes {
  id: string;
  name: string;
...
export class OktaSAMLApplicationClient {
  private attributes: OktaSAMLApplicationClientAttributes;

  constructor(attrs?: Partial<OktaSAMLApplicationClientAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'OktaSAMLApplicationClient Standard Configuration',
  ...
  public setState(state: OktaSAMLApplicationClientAttributes['state']): this {
  ...
  public toJSON(): OktaSAMLApplicationClientAttributes {

/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: EVENTSTORESNAPSHOT
 * ============================================================================
 * Description: Event store aggregate snapshot with version number
...
export interface EventStoreSnapshotAttributes {
  id: string;
  name: string;
...
export class EventStoreSnapshot {
  private attributes: EventStoreSnapshotAttributes;

  constructor(attributes?: Partial<EventStoreSnapshotAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attributes?.id || generateUUID(),
      name: attributes?.name || 'EventStoreSnapshot Instance',
  ...
  public update(attributes: Partial<EventStoreSnapshotAttributes>): this {
    this.attributes = {
      ...this.attributes,
      ...attributes,
  ...
  public setStatus(status: EventStoreSnapshotAttributes['status']): this {
  ...
  public toJSON(): EventStoreSnapshotAttributes {
    return { ...this.attributes };
  }

  public static fromJSON(json: Partial<EventStoreSnapshotAttributes>): EventStoreSnapshot {
    return new EventStoreSnapshot(json);
  }

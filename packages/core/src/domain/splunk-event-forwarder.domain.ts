/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: SPLUNKEVENTFORWARDER
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface SplunkEventForwarderAttributes {
  id: string;
  name: string;
...
export class SplunkEventForwarder {
  private attributes: SplunkEventForwarderAttributes;

  constructor(attrs?: Partial<SplunkEventForwarderAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'SplunkEventForwarder Standard Configuration',
  ...
  public setState(state: SplunkEventForwarderAttributes['state']): this {
  ...
  public toJSON(): SplunkEventForwarderAttributes {

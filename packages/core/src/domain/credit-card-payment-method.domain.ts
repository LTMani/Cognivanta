/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: CREDITCARDPAYMENTMETHOD
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface CreditCardPaymentMethodAttributes {
  id: string;
  name: string;
...
export class CreditCardPaymentMethod {
  private attributes: CreditCardPaymentMethodAttributes;

  constructor(attrs?: Partial<CreditCardPaymentMethodAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'CreditCardPaymentMethod Default Item',
  ...
  public setStatus(status: CreditCardPaymentMethodAttributes['status']): this {
  ...
  public toJSON(): CreditCardPaymentMethodAttributes {

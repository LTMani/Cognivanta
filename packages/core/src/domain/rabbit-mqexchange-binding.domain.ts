/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: RABBITMQEXCHANGEBINDING
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface RabbitMQExchangeBindingAttributes {
  id: string;
  name: string;
...
export class RabbitMQExchangeBinding {
  private attributes: RabbitMQExchangeBindingAttributes;

  constructor(attrs?: Partial<RabbitMQExchangeBindingAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'RabbitMQExchangeBinding Standard Configuration',
  ...
  public setState(state: RabbitMQExchangeBindingAttributes['state']): this {
  ...
  public toJSON(): RabbitMQExchangeBindingAttributes {

/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: DLPINCIDENTRECORD
 * ============================================================================
 * Description: Data loss prevention incident record with masked tokens
...
export interface DLPIncidentRecordAttributes {
  id: string;
  name: string;
...
export class DLPIncidentRecord {
  private attributes: DLPIncidentRecordAttributes;

  constructor(attributes?: Partial<DLPIncidentRecordAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attributes?.id || generateUUID(),
      name: attributes?.name || 'DLPIncidentRecord Instance',
  ...
  public update(attributes: Partial<DLPIncidentRecordAttributes>): this {
    this.attributes = {
      ...this.attributes,
      ...attributes,
  ...
  public setStatus(status: DLPIncidentRecordAttributes['status']): this {
  ...
  public toJSON(): DLPIncidentRecordAttributes {
    return { ...this.attributes };
  }

  public static fromJSON(json: Partial<DLPIncidentRecordAttributes>): DLPIncidentRecord {
    return new DLPIncidentRecord(json);
  }

/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: GCPKMSCRYPTOKEYVERSION
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface GCPKMSCryptoKeyVersionAttributes {
  id: string;
  name: string;
...
export class GCPKMSCryptoKeyVersion {
  private attributes: GCPKMSCryptoKeyVersionAttributes;

  constructor(attrs?: Partial<GCPKMSCryptoKeyVersionAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'GCPKMSCryptoKeyVersion Standard Configuration',
  ...
  public setState(state: GCPKMSCryptoKeyVersionAttributes['state']): this {
  ...
  public toJSON(): GCPKMSCryptoKeyVersionAttributes {

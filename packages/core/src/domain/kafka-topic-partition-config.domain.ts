/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: KAFKATOPICPARTITIONCONFIG
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface KafkaTopicPartitionConfigAttributes {
  id: string;
  name: string;
...
export class KafkaTopicPartitionConfig {
  private attributes: KafkaTopicPartitionConfigAttributes;

  constructor(attrs?: Partial<KafkaTopicPartitionConfigAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'KafkaTopicPartitionConfig Standard Configuration',
  ...
  public setState(state: KafkaTopicPartitionConfigAttributes['state']): this {
  ...
  public toJSON(): KafkaTopicPartitionConfigAttributes {

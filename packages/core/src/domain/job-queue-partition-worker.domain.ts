/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: JOBQUEUEPARTITIONWORKER
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface JobQueuePartitionWorkerAttributes {
  id: string;
  name: string;
...
export class JobQueuePartitionWorker {
  private attributes: JobQueuePartitionWorkerAttributes;

  constructor(attrs?: Partial<JobQueuePartitionWorkerAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'JobQueuePartitionWorker Default Item',
  ...
  public setStatus(status: JobQueuePartitionWorkerAttributes['status']): this {
  ...
  public toJSON(): JobQueuePartitionWorkerAttributes {

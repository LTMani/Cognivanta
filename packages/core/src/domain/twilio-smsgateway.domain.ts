/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: TWILIOSMSGATEWAY
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface TwilioSMSGatewayAttributes {
  id: string;
  name: string;
...
export class TwilioSMSGateway {
  private attributes: TwilioSMSGatewayAttributes;

  constructor(attrs?: Partial<TwilioSMSGatewayAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'TwilioSMSGateway Default Item',
  ...
  public setStatus(status: TwilioSMSGatewayAttributes['status']): this {
  ...
  public toJSON(): TwilioSMSGatewayAttributes {

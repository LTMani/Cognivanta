/**
 * ============================================================================
 * COGNIVANTA DOMAIN MODEL: HELMRELEASEDEPLOYMENT
 * ============================================================================
 */

import { generateUUID, sha256 } from '../utils';

export interface HelmReleaseDeploymentAttributes {
  id: string;
  name: string;
...
export class HelmReleaseDeployment {
  private attributes: HelmReleaseDeploymentAttributes;

  constructor(attrs?: Partial<HelmReleaseDeploymentAttributes>) {
    const now = new Date().toISOString();
    this.attributes = {
      id: attrs?.id || generateUUID(),
      name: attrs?.name || 'HelmReleaseDeployment Standard Configuration',
  ...
  public setState(state: HelmReleaseDeploymentAttributes['state']): this {
  ...
  public toJSON(): HelmReleaseDeploymentAttributes {

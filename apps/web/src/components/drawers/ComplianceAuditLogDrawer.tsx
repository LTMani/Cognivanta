...
export interface ComplianceAuditLogDrawerProps {
  isOpen: boolean;
  onClose: () => void;
  item?: Record<string, unknown>;
}

export const ComplianceAuditLogDrawer: React.FC<ComplianceAuditLogDrawerProps> = ({ isOpen, onClose, item }) => {
  return (
    <Drawer isOpen={isOpen} onClose={onClose} title="ComplianceAuditLog Inspector">
      <div className="space-y-6">
        <div className="p-4 bg-surface-300 rounded-2xl border border-card-border">
          ...
          <p className="text-lg font-bold text-slate-100 mt-1">{String(item?.name || 'ComplianceAuditLog Instance')}</p>

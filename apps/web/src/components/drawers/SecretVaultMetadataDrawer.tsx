...
export interface SecretVaultMetadataDrawerProps {
  isOpen: boolean;
  onClose: () => void;
  item?: Record<string, unknown>;
}

export const SecretVaultMetadataDrawer: React.FC<SecretVaultMetadataDrawerProps> = ({ isOpen, onClose, item }) => {
  return (
    <Drawer isOpen={isOpen} onClose={onClose} title="SecretVaultMetadata Inspector">
      <div className="space-y-6">
        <div className="p-4 bg-surface-300 rounded-2xl border border-card-border">
          ...
          <p className="text-lg font-bold text-slate-100 mt-1">{String(item?.name || 'SecretVaultMetadata Instance')}</p>

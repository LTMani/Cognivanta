...
export interface CreateNotificationAlertModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit?: (data: Record<string, unknown>) => void;
}

export const CreateNotificationAlertModal: React.FC<CreateNotificationAlertModalProps> = ({ isOpen, onClose, onSubmit }) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Configure NotificationAlert">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="block text-xs font-semibold text-slate-300 mb-1.5">NotificationAlert Name</label>
          <Input
            required
            value={name}
        ...
        <div className="pt-4 border-t border-card-border flex justify-end space-x-3">
          ...
          <Button type="submit" variant="primary" size="sm" disabled={isLoading}>
            {isLoading ? 'Creating...' : 'Create NotificationAlert'}
          </Button>
        </div>
      </form>
    </Modal>

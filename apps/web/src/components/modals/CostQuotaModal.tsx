...
interface CostQuotaModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: Record<string, unknown>) => void;
}

export const CostQuotaModal: React.FC<undefined> = ({ isOpen, onClose, onSubmit }) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Set Organization Token Quotas">
      <form onSubmit={handleSubmit} className="space-y-4">
        <p className="text-sm text-slate-400">
          Modal for establishing monthly budget caps, token limits, and alert webhooks.
        </p>

...
export const SMTPGatewaysView: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-100">SMTPGateways Explorer</h2>
          <p className="text-xs text-slate-400 mt-1">Enterprise management and configuration for SMTPGateways.</p>
        </div>
            ...
            <tbody className="divide-y divide-card-border/50">
              {[1, 2, 3, 4, 5].map(i => (
                <tr key={i} className="hover:bg-surface-200/50 transition-colors">
                  <td className="py-2.5 px-3 font-medium text-slate-100">SMTPGateways-item-${i}</td>
